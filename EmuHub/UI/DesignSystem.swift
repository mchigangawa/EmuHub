...
/// A compact glass capsule button that expands to reveal a label on hover.
struct GlassCapsuleButton: View {
    let systemImage: String
    let label: String
    var tint: Color = .accentColor
    /// When true the capsule is filled with the tint (primary action).
    ...
    var body: some View {
        Button(action: action) {
            ...
            .background {
                if prominent {
                    Capsule().fill(Accent.gradient(tint))
                        .shadow(color: hovered ? tint.opacity(0.4) : .clear, radius: 5, y: 2)
                } else {
